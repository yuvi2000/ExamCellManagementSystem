...
public class HomeDAOImpl implements HomeDAO {
	  ...
	  public int update(NominalRole p){  
		...
		public void register1(Papers subject){
		  ...
		  public Papers showSubjects(Papers subject) {
		    ...
		    String sql = "select num_trn_paper_id, num_degree_id, num_dept_id, num_year_id, num_semester_id, num_paper_id from tbl_trn_papers";// where num_Student_Id=" + student.getStudentId() ;//" and vch_student_fname='" + student.getFirstname() + "'";
		  ...
		  public int update1(Papers p){  
			    String sql="update tbl_trn_papers set num_degree_id="+p.getDegree()+", num_dept_id="+p.getDept()+", num_year_id="+p.getYear()+", num_semester_id="+p.getSemester()+", num_paper_id="+p.getPaper()+"";
			...
			public Papers getPapersById(int semester){  
			    String sql="select num_trn_paper_id, num_degree_id, num_dept_id, num_year_id, num_semester_id, num_paper_id from tbl_trn_papers where num_trn_paper_id=?";  
			...
			public List<Papers> getAllPapers(){  
			    return jdbcTemplate.query("select num_trn_paper_id, num_degree_id, num_dept_id, num_year_id, num_semester_id, num_paper_id  from tbl_trn_papers",new RowMapper<Papers>(){  
			        public Papers mapRow(ResultSet rs, int row) throws SQLException {  
			        	Papers e=new Papers();  
			            e.setPaperId(rs.getInt(1));
			        	e.setDegree(rs.getInt(2));  
			            e.setDept(rs.getInt(3));  
			            e.setYear(rs.getInt(4));  
			            e.setSemester(rs.getInt(5));  
			            e.setPaper(rs.getInt(6));
	...
	class PapersMapper implements RowMapper<Papers> {
	  ...
	  public Papers mapRow(ResultSet rs, int arg1) throws SQLException {
		  ...
		  subject.setSemester(rs.getInt("num_semester_id"));
